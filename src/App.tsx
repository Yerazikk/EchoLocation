...
export default function App() {
  ...
  // Fixed display size — computed once from image aspect ratio, never changes on resize
    ...
    if (timeline.isPlaying) {
      timerRef.current = setInterval(() => {
        setTimeline(prev => {
          ...
          // Clip logic: pause at end of clip
  ...
  // Audio Event Simulation Logic
  useEffect(() => {
    const currentTime = timeline.currentTime;
    
    // Reset triggered events if timeline is reset
    if (currentTime === 0) {
    ...
    if (newEvents.length > 0) {
      ...
      setActiveEvents(prev => {
        ...
        const eventsWithExpiry = newEvents.map((e, idx) => {
          // Calculate stack offset based on how many events are currently active for this node
          const existingCount = filtered.filter(ev => ev.nodeId === e.nodeId).length;
          return {
            ...e,
            expiresAt: now + 4000, // Increased duration for smoother fade
            initialOffset: (existingCount + idx) * -28 // Slightly more spacing
          };
        });
  ...
  // Sync sidebar video to timeline
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-screen bg-ui-bg text-white font-sans selection:bg-ui-accent selection:text-black">
      {/* Header */}
      <header className="h-16 border-b border-ui-border flex items-center justify-between px-6 bg-ui-bg/80 backdrop-blur-md z-50">
      ...
      <div className="flex-1 flex overflow-hidden">
        <main className="flex-1 relative overflow-hidden flex flex-col">
          {/* Map Area — fixed size map centered; surrounding space shrinks on resize */}
          <div className="flex-1 flex items-center justify-center overflow-hidden bg-black/20">
            {mapImage ? (
              /* Map coordinate space: fixed pixel size, shared by image/grid/nodes */
              <div
                ref={mapContainerRef}
              ...
              >
                {/* Blueprint Image — fills the coordinate space exactly */}
                <img
                  src={mapImage}
                  alt="Floor Plan"
                ...
                {/* Grid Overlay — same coordinate space as image */}
                {showGrid && (
                  ...
                  {nodes.map((node) => {
                    ...
                    >
                      ...
                      >
                        ...
                        {/* Hover Preview Panel — not shown for damaged camera */}
                        <AnimatePresence>
        ...
        <aside className="w-80 border-l border-ui-border bg-ui-bg/50 backdrop-blur-2xl flex flex-col h-full z-50">
          {/* Top Section: Live Feed / Now Playing */}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-black/10">
              {selectedNodeId ? (
                (() => {
                  // History includes all events triggered so far, even if we rewind
      ...
      {/* Playback Controls */}
      <footer className="h-24 border-t border-ui-border bg-ui-bg/90 backdrop-blur-xl px-8 flex items-center gap-8 z-50">
        <div className="flex items-center gap-4">
          <button 
            onClick={togglePlay}
